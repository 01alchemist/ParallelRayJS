...
export class Vec3f {
    ...
    set(x:number|Vec3f, y?:number, z?:number):Vec3f {
        let v = x instanceof Vec3f ? x : null;
        this.x = v.x || x;
        this.y = v.y || y;
        this.z = v.z || z;
        return this;
    ...
    add(f:Vec3f | number):Vec3f {
        let v = f instanceof Vec3f ? f : null;
        return new Vec3f(this.x + (v.x || f), this.y + (v.y || f), this.z + (v.z || f));
    }

    sub(f:Vec3f | number):Vec3f {
        let v = f instanceof Vec3f ? f : null;
        return new Vec3f(this.x - (v.x || f), this.y - (v.y || f), this.z - (v.z || f));
    }

    scale(f:Vec3f|number):Vec3f {
        let v = f instanceof Vec3f ? f : null;
        return new Vec3f(this.x * (v.x || f), this.y * (v.y || f), this.z * (v.z || f));
    }

    divide(f:Vec3f|number):Vec3f {
        let v = f instanceof Vec3f ? f : null;
        return new Vec3f(this.x / (v.x || f), this.y / (v.y || f), this.z / (v.z || f));
    }
